...
- A small `ConnectionStatusToast` component listens for `window` `online`/`offline` events and shows a toast when connectivity is lost; the "back online" toast only appears if the offline toast was shown first.
